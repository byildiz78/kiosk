...
const totalPrice = computed(() => {
  let total = basket.value.orderType === "TakeOut"
  ...
  if (props.selectedProduct?.Combo) {
    props.selectedProduct.Combo.forEach((group) => {
      group.Items.forEach((item) => {
        ...
        if (selectedItem) {
          const extraPrice = basket.value.orderType === "TakeOut"
          ...
          if (selectedSubItem) {
            const subItemExtraPrice = basket.value.orderType === "TakeOut"
...
const canDeselectItem = (group, item) => {
  if (group.IsForcedGroup && group.MaxQuantity === 1 && group.Items.length === 1 && isItemSelected(item)) {
    return false
  }
  return true
}
...
const getSelectedItemsArray = () => {
  ...
  selectedItems.value.forEach((value, key) => {
    value.item.price = (basket.value.orderType === "TakeOut"
